...
> 0.1.6 — профиль перезапускается автоматически после смены настроек, а состояние службы восстанавливается после пересоздания Android-сервиса. Подписанный APK публикуется в Releases после автоматических проверок. Работа обхода на конкретном телефоне и у конкретного провайдера требует проверки.
